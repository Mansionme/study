<template>
  <div class="container">
    <div class="title">科室计划信息</div>
    <!-- 面包屑导航区 -->
    <el-breadcrumb separator-class="el-icon-arrow-right">
      <el-breadcrumb-item :to="{ path: '/' }">首页</el-breadcrumb-item>
      <el-breadcrumb-item :to="{ path: '/department' }">科室信息管理</el-breadcrumb-item>
      <el-breadcrumb-item>科室计划信息</el-breadcrumb-item>
    </el-breadcrumb>
...
export default {
  data () {
    ...
    return {
      ...
      addRules: {
        subject: [
          { required: true, message: '请输入计划标题(中文)', trigger: ['blur', 'change'] },
          { validator: validate }
        ]
        // remarks: [
        //   { required: true, message: '请输入备注(中文)', trigger: ['blur', 'change'] },
        //   { validator: validate }
        // ]
      }
    }
  ...
  methods: {
    ...
    async getDepartment (id) {
      const { data: res } = await this.$http.get('/api/department/query/' + this.$route.query.id)
      this.depart_value = res.result.id
    },
